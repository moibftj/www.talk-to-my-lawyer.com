...
const LETTER_TYPES: Record<string, { name: string; description: string; tone: string }> = {
  'demand': {
    name: 'Demand Letter',
  ...
  'complaint': {
    ...
    description: 'A formal letter expressing dissatisfaction and seeking resolution',
    tone: 'professional, factual, and solution-oriented'
  },
  'dispute': {
    name: 'Dispute Letter',
    description: 'A letter challenging or disputing a claim, charge, or decision',
...
function buildUserPrompt(request: LetterGenerationRequest): string {
  let prompt = `Please draft a professional legal letter with the following details:
...
async function generateLetter(request: LetterGenerationRequest): Promise<LetterGenerationResult> {
  if (!OPENAI_API_KEY) {
  ...
  if (!letterTypeConfig) {
    return { success: false, error: `Invalid letter type: ${request.letterType}` }
  }
  ...
  try {
    ...
    console.log('[GenerateLetter] Letter generated successfully in', duration, 'ms')
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (subError || !subscription) {
      return new Response(
        JSON.stringify({ error: 'Active subscription required', code: 'NO_SUBSCRIPTION' }),
        { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }

    const remainingCredits = subscription.remaining_letters || subscription.credits_remaining || 0
    if (remainingCredits <= 0) {
      return new Response(
        JSON.stringify({ error: 'No letter credits remaining', code: 'NO_CREDITS' }),
        { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    ...
    if (!letterRequest.letterType || !letterRequest.recipientName || 
        !letterRequest.subject || !letterRequest.situation || 
        !letterRequest.desiredOutcome || !letterRequest.senderName) {
      return new Response(
        JSON.stringify({ 
          error: 'Missing required fields', 
          required: ['letterType', 'recipientName', 'subject', 'situation', 'desiredOutcome', 'senderName'] 
        }),
